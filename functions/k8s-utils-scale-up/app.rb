...
FunctionsFramework.cloud_event "k8s-utils-scale-up" do |event|
  ...
  apps_client.patch_stateful_set("game", { spec: { replicas: 1 } }, app_name)

  client = Google::Cloud::PubSub.new
  topic = client.topic(TOPIC)
  topic.publish(follow_up_options.merge("app_name" => app_name).to_json)

  update_discord_message(<<~MSG, **follow_up_options)
    Scaling up server `#{app_name}` succeeded.
  MSG
